...
IOS_APP_VERSION=$(sed -n '/MARKETING_VERSION/{s/MARKETING_VERSION = //;s/;//;s/^[[:space:]]*//;p;q;}' ios/joinforma.xcodeproj/project.pbxproj)
IOS_BUILD_NUMBER=$(sed -n '/CURRENT_PROJECT_VERSION/{s/CURRENT_PROJECT_VERSION = //;s/;//;s/^[[:space:]]*//;p;q;}' ios/joinforma.xcodeproj/project.pbxproj)

APP_VERSION="ir-$IOS_APP_VERSION-b$IOS_BUILD_NUMBER"
