import * as React from "react";
import Svg, { Path } from "react-native-svg";

import Colors from "Themes/Colors";

const ExternalLinkSvgIcon = (props) => {
  const { fillColor = Colors.black, height = 20, width = 20, strokeWidth = 2, style = {} } = props;

  return (
    <Svg viewBox="0 0 20 20" fill="none" width={width} height={height} style={style}>
      <Path
        d="M15 10.8333V15.8333C15 16.2754 14.8244 16.6993 14.5118 17.0118C14.1993 17.3244 13.7754 17.5 13.3333 17.5H4.16667C3.72464 17.5 3.30072 17.3244 2.98816 17.0118C2.67559 16.6993 2.5 16.2754 2.5 15.8333V6.66667C2.5 6.22464 2.67559 5.80072 2.98816 5.48816C3.30072 5.17559 3.72464 5 4.16667 5H9.16667"
        stroke={fillColor}
        strokeWidth={strokeWidth}
      />
      <Path d="M12.5 2.5H17.5V7.5" stroke={fillColor} strokeWidth={strokeWidth} />
      <Path d="M8.33331 11.6667L17.5 2.5" stroke={fillColor} strokeWidth={strokeWidth} />
    </Svg>
  );
};
export default ExternalLinkSvgIcon;
